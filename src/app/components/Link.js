  ...
  return (
    <Link href={href}>
      <Button className="rounded pt-5 pr-0 pb-5 pl-0 flex flex-row gap-3 items-center justify-center  relative text-2xl font-bold cursor-pointer">
        ...
        <div className="text-white">Link teilen</div>
      </Button>
    </Link>
  );
};
...
export const ShowImages = ({ href, ...props }) => {
  ...
  return (
    <>
    {showQR && (
        <div onClick={() => setShowQR((s) => !s)} class="absolute top-0 left-0 w-screen h-screen  flex justify-center items-center bg-white bg-opacity-30">
        ...
        <Button onClick={() => setShowQR((s)=>!s)} className="rounded pt-5 pr-0 pb-5 pl-0 flex flex-row gap-3 items-center justify-center  relative text-2xl font-bold cursor-pointer">
        <svg
          className="w-8"
          viewBox="0 0 24 24"
          fill="none"
